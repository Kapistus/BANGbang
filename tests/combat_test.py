...
Two clients on tests/range.map, one shooting the other, checking the things that
...
Run from the project root:   python -m tests.combat_test
"""
import math
import time
...
def main():
    ...
    srv = GameServer([a_pos, b_pos], port=PORT, map_id=R.MAP_ID,
...
def glass_test():
    ...
    for (r, c), _ in [((r, c), None)
        ...
        if pane:
            break
    if pane is None:
        print("no glass on the range to shoot, skipped")
        return
    (gx, gy), (sx, sy) = pane
    print(f"glass at ({gx:.1f}, {gy:.1f}), shooting from ({sx:.1f}, {sy:.1f})")

    srv = GameServer([(sx, sy), (sx, sy)], port=PORT + 1, map_id=R.MAP_ID,
...
def door_test():
    ...
    m, _ = netmaps.load_with_spawns(R.MAP_ID, R.MAPS_DIR)
    doors = find_doors(m)
    assert doors, "no doors on the range to test with"
    ...
    srv = GameServer([stand, stand], port=PORT + 2, map_id=R.MAP_ID,
                     maps_dir=R.MAPS_DIR, duration_s=300)
    srv.start()
