...
which is what breaks silently when the sim or the protocol moves underneath it.
    ...
    python -m tests.mp_smoke_test                  # on tests/range.map
    python -m tests.mp_smoke_test vessel_interior  # or any other map id
"""
import math
import os
import sys
...
# the viewport a 1920x1080 screen gets (main.view_cap), pinned here so the
# frame-rate figures below are the ones people actually play at rather
# than whatever desktop size the dummy video driver reports
PLAY_VIEW = (1860, 960)

PORT = 47997
RUN_S = 6.0                  # how long to drive, in real seconds
# a map id from maps/ on the command line, otherwise the test range
MAP = sys.argv[1] if len(sys.argv) > 1 else R.MAP_ID
MAPS_DIR = "maps" if len(sys.argv) > 1 else R.MAPS_DIR
...
def main():
    ...
    m, spawns = netmaps.load_with_spawns(MAP, MAPS_DIR)
    ...
    srv = GameServer([open_spot, spawns[-1]], port=PORT, map_id=MAP,
                     maps_dir=MAPS_DIR, duration_s=120)
    srv.start()
    ...
    pygame.init()
    view = mp_client.MatchView(a, MAPS_DIR, window=PLAY_VIEW)
...
def trim_order_test():
    """When the active-sound list is full, what goes is a sound the listener
    has already heard, not one still on its way. Two shooters make the
    difference: the near one's newer shots arrive at once, the far one's older
    shot is still crossing the map, and by age alone the far one was dropped
    unheard."""
    import main as renderer

    class Snd:
        def __init__(self, name, cued):
            self.name, self.cued, self.field = name, cued, object()
    far = Snd("far, fired first, still travelling", False)
    near = [Snd(f"near {i}, already heard", True) for i in range(3)]
    sounds = [far] + near
    renderer._trim_sounds(sounds, None, None, 3)
    print(f"  full list, one over: dropped the "
          f"{'far unheard shot' if far not in sounds else 'oldest heard shot'}")
    assert far in sounds, "the cap dropped a sound before it reached the listener"
    everything_travelling = [Snd(str(i), False) for i in range(4)]
    renderer._trim_sounds(everything_travelling, None, None, 3)
    assert [x.name for x in everything_travelling] == ["1", "2", "3"], \
        "with nothing heard yet, the oldest should still be the one to go"
    print("\nTRIM ORDER CHECKS PASSED")
...
def sound_reuse_test():
    ...
    # the listener at one end of the hall, the shooter at the other: far
    # enough that dropping sounds by age alone would lose shots on the way
    a_pos, b_pos = R.FAR_PAIR
    srv = GameServer([a_pos, b_pos], port=PORT + 3, map_id=R.MAP_ID,
    ...
    pygame.init()
    view = mp_client.MatchView(a, R.MAPS_DIR, window=PLAY_VIEW)
    ...
    # every sound the cap pushes out: was it one the listener had not heard yet?
    unheard_drops = {"n": 0, "cued": 0}
    real_trim = renderer._trim_sounds

    def watching_trim(sounds, jobs, cache, max_active):
        before = list(sounds)
        real_trim(sounds, jobs, cache, max_active)
        for gone in (snd for snd in before if snd not in sounds):
            unheard_drops["cued" if gone.cued else "n"] += 1
    renderer._trim_sounds = watching_trim
    ...
    try:
        ...
        print(f"  the cap let go of {unheard_drops['cued']} already-heard "
              f"sound(s) and {unheard_drops['n']} still on their way")
        assert unheard_drops["n"] == 0, \
            "the cap dropped sounds before they reached the listener: " \
            "sustained fire goes silent at range"
        ...
        renderer._trim_sounds = real_trim
        a.disconnect()
        b.disconnect()
        srv.stop()
...
def late_join_test():
    ...
    tests/rejoin_test.py covers the server side of this. What is checked here is
    ...
    srv = GameServer(spawns, port=PORT + 4, map_id=R.MAP_ID,
    ...
    try:
        ...
        # --- connect mid-match: the lobby asks for a class, then puts us
        # straight in
        late = GameClient("127.0.0.1", PORT + 4)
        assert late.connect("Carol", (40, 200, 60)), late.reject_reason
        lob = L.Lobby(late, fonts, maps_dir=R.MAPS_DIR, auto_join=True)
        t0 = time.monotonic()
        while time.monotonic() - t0 < 0.6:
            lob.pump_events()
            lob.draw(surf)
            time.sleep(1 / 60)
        assert lob.choosing_class and lob.result is None \
            and not late.world.playing, \
            "a latecomer was dropped in before choosing a class"
        print("  fresh connection during a match -> asked for a class first")
        card = lob._card_rects()[1]
        lob.handle_event(pygame.event.Event(pygame.MOUSEBUTTONDOWN, button=1,
                                            pos=card.center))
        assert not lob.choosing_class, "clicking a class card did nothing"
        t0 = time.monotonic()
        ...
        t0 = time.monotonic()
        while (late.world.players[late.world.my_id].cls_now != "heavy_support"
               and time.monotonic() - t0 < 2.0):
            time.sleep(0.05)
        assert late.world.players[late.world.my_id].cls_now == "heavy_support", \
            "joined as something other than the class picked"
        print("  ...and joined as the class picked on the card")

        # --- the match render must inherit the door that is already open
        view = mp_client.MatchView(late, R.MAPS_DIR, window=PLAY_VIEW)
        sub_n = view.m.subdiv
        ...
        lob2 = L.Lobby(late, fonts, maps_dir=R.MAPS_DIR, auto_join=False)
        t0 = time.monotonic()
...
def lighting_test():
    ...
    srv = GameServer([a_pos, b_pos], port=PORT + 5, map_id=R.MAP_ID,
                     maps_dir=R.MAPS_DIR, duration_s=300)
    srv.start()
    ...
    try:
        ...
        view.read_input = lambda: (0.0, 0.0, 0)
        # every lamp off: whatever this corner of the range happens to be lit
